...
//alert("_v:"+_v+"-_d:"+_d);
	...
	if( s[0].length == 0 ) s.splice(0,1);
	// convert the string into an array in which digits are together
	var tmpe = v.split(/[^0-9]/);

	//ASM: String.split() with regular expressions is different in IE and Mozilla/Firefox
	//We add the following code for Netscape
	var ie = ((document.all) && (window.offscreenBuffering)) ? true : false;
    if(!ie) {
    	e = new Array();
		var e_idx = 0;
		for(i = 0; i < tmpe.length; i++) {
			if(tmpe[i] != "") {
				e[e_idx] = tmpe[i];
				e_idx++;
			}
