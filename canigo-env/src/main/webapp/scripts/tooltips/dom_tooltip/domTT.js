/** $Id: domTT.js,v 1.1 2007/05/23 11:11:18 msabates Exp $ */
// {{{ license
...
// EVC: added to know if the selects are opened or not.
// var selectsFlag = new Hash();
var domTT_comeFromHTMLOptionElement = false;
...
{
	// EVC: FF HTMLOptionElement coords bug fixed.
	if ((in_event.target instanceof HTMLSelectElement) 
		&& (domTT_comeFromHTMLOptionElement)){ 
		domTT_comeFromHTMLOptionElement = false;
		return false; 
	}
	if ((in_event.target instanceof HTMLOptionElement)){ 
		domTT_comeFromHTMLOptionElement = true;
		return false; 
	}
	domTT_comeFromHTMLOptionElement = false;
...
//$('myDiv').innerHTML = "mouseX:" + mouseX + " mouseY:" + mouseY;
	...
	if (tooltip.get('position') == 'absolute')
	{
		// EVC: modified to show all the hidden selects
		//domLib_detectCollisions(tipObj);
		domLib_detectCollisions(tipObj,null,true);
	}
}
